...
public class UIManager : MonoBehaviourSingleton<UIManager>
{
    ...
    private void Start()
    {
        ...
        // ✅ Ensure network UI buttons are assigned
        if (hostButton != null)
            hostButton.onClick.AddListener(() => GameManager.Instance.StartHost());
        else
            Debug.LogError("UIManager: hostButton is not assigned!");


        if (serverButton != null)
            serverButton.onClick.AddListener(() => GameManager.Instance.StartServer());
        else
            Debug.LogError("UIManager: serverButton is not assigned!");

        if (statusText == null)
            Debug.LogError("UIManager: statusText is not assigned!");

        // ✅ Ensure NetworkManager exists before accessing it
        if (NetworkManager.Singleton != null)
        {
            NetworkManager.Singleton.OnClientConnectedCallback += UpdateConnectionStatus;
            NetworkManager.Singleton.OnClientDisconnectCallback += UpdateConnectionStatus;
        }
        else
        {
            Debug.LogError("UIManager: NetworkManager.Singleton is null!");
        }
    }
    ...
    /// </summary>
    private void UpdateConnectionStatus(ulong clientId)
    {
        if (NetworkManager.Singleton.IsServer)
        {
            int connectedPlayers = NetworkManager.Singleton.ConnectedClientsList.Count;
            statusText.text = $"Connected Players: {connectedPlayers}";

            // ✅ Send the updated status to all clients
            UpdateClientStatusTextClientRpc(connectedPlayers);
        }
    }

    [ClientRpc]
    public void UpdateClientStatusTextClientRpc(int connectedPlayers)
    {
        Debug.Log($"✅ ClientRpc received: Updating UI with {connectedPlayers} connected players.");
    ...
    public void JoinSession()
    {
        ...
        {
            ShowError("⚠ Already in a session!");
            return;
        }

        Debug.Log($"✅ Attempting to join session...");

        if (NetworkManager.Singleton.StartClient())
        {
            statusText.text = $"🔄 Joining game..."; // Initial status
            NetworkManager.Singleton.OnClientConnectedCallback += OnClientSuccessfullyJoined;
        }
        else
        {
    ...
    private void OnClientSuccessfullyJoined(ulong clientId)
    {
        Debug.Log($"✅ Client {clientId} successfully joined!");
        statusText.text = $"✅ Connected to game!";
        NetworkManager.Singleton.OnClientConnectedCallback -= OnClientSuccessfullyJoined; // Unsubscribe to avoid duplicate calls
    }
    ...
    public void Reconnect()
    {
        ...
        GameManager.Instance.ReconnectPlayer(clientId);
    }
