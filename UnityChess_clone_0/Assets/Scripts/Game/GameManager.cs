...
public class GameManager : MonoBehaviourSingleton<GameManager>
{
    ...
    // Dictionary to track connected players (for managing rejoining and disconnects)
    ...
    private void Start()
    {
        // ✅ Subscribe to visual piece movement events
    ...
    /// <summary>
    /// Starts a client to join a hosted game.
    /// </summary>
    ...
    private void OnClientConnected(ulong clientId)
    {
        ...
        {
            playerConnections[clientId] = true;
        }

        // 🛠 FIX: Only access ConnectedClientsList if we are on the SERVER
        if (NetworkManager.Singleton.IsServer)
        {
            int playerCount = NetworkManager.Singleton.ConnectedClientsList.Count;
            UIManager.Instance.UpdateClientStatusTextClientRpc(playerCount);

            // Start the game if at least 2 players are connected
            if (playerCount >= 2)
            {
                Debug.Log("🎮 Minimum players reached. Starting game...");
                StartNewGame();
            }
        }
    }
    ...
    private void RequestConnectedPlayersServerRpc(ServerRpcParams rpcParams = default)
    {
        if (!NetworkManager.Singleton.IsServer)
        {
            Debug.LogWarning("⚠ RequestConnectedPlayersServerRpc() called from a client! Ignoring request.");
            return;
        }
        ...
        // Send the correct status to the requesting client
        UIManager.Instance.UpdateClientStatusTextClientRpc(connectedPlayers);
    }
    ...
    private void OnClientDisconnected(ulong clientId)
    {
        ...
        if (playerConnections.ContainsKey(clientId))
        {
            playerConnections[clientId] = false;
        }

        int connectedPlayers = NetworkManager.Singleton.ConnectedClientsList.Count;
        UIManager.Instance.UpdateClientStatusTextClientRpc(connectedPlayers);

        if (connectedPlayers < 2 && NetworkManager.Singleton.IsServer)
        {
            Debug.Log("⚠ Not enough players, game halted.");
        }
    }
    ...
    public void ReconnectPlayer(ulong clientId)
    {
        if (playerConnections.ContainsKey(clientId) && !playerConnections[clientId])
        {
            Debug.Log($"🔄 Player {clientId} is rejoining...");
            playerConnections[clientId] = true;
            SyncGameStateClientRpc(SerializeGame());
        }
    }
